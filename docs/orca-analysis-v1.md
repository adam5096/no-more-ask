# 🎭 節慶互助平台 - ORCA 分析文檔 v1.0

> **版本說明**：第一代 ORCA 分析，基於 blue-print 的初步對象識別與關係定義  
> **建立日期**：2024  
> **狀態**：探索階段 - 待迭代優化  
...
1. **User 與 Helper 的關係**
   - ❓ 一個 User 是否只能有一個 Helper 身份？還是可以同時有多個角色？
   - 💡 建議：一個 User 可以有多個角色，但 Helper 身份是獨立的資料表

2. **RescueRequest 的匹配邏輯**
   - ❓ 一個 RescueRequest 是否可以匹配多個 Helper（讓用戶選擇）？
   - 💡 建議：先匹配多個候選，用戶選擇後才正式匹配

3. **Gathering 與 VentPost 的關係**
   - ❓ VentPost 是否必須關聯到 Gathering？還是可以獨立存在？
   - 💡 建議：VentPost 可以獨立，也可以選擇關聯到 Gathering

### 關係層級問題

4. **MapLayer 的數據來源**
   - ❓ MapLayer 是獨立的對象，還是其他對象的視圖？
   - 💡 建議：MapLayer 是視圖層，數據來源於其他對象（RescueRequest, Helper, Gathering）

5. **Notification 的觸發機制**
   - ❓ 哪些操作會觸發 Notification？是否需要通知偏好設定？
   - 💡 建議：建立 NotificationRule 對象來管理觸發規則

### 功能層級問題

6. **AI 腳本生成的限制**
   - ❓ ResponseScript 是否需要保存歷史記錄？還是每次都是新生成？
   - 💡 建議：保存歷史記錄，允許用戶收藏常用腳本

7. **診斷報告的分享機制**
   - ❓ DiagnosticReport 的分享是公開還是私密連結？
   - 💡 建議：使用 token 生成私密連結，可設定過期時間

### 屬性層級問題

8. **地理位置的精確度**
   - ❓ 是否需要精確座標，還是只需要大致區域？
   - 💡 建議：根據功能需求，RescueRequest 需要精確座標，MapLayer 可以模糊化

9. **匿名機制的實作**
   - ❓ VentPost 匿名時，userId 是否還需要保存？
   - 💡 建議：保存 userId 用於後台管理，但對外顯示為匿名

10. **Line 整合的範圍**
    - ❓ 是否所有通知都要發送到 Line？還是可選？
    - 💡 建議：用戶可設定通知偏好，選擇哪些類型要發送到 Line
...
12. **頁面複雜度判斷**
    - ❓ 哪些頁面是 Simple（1:1）？哪些是 Standard（1:2-3）？哪些是 Complex（1:N）？
    - 💡 建議：
      - Simple：邊界說明書頁面、應對錦囊頁面
      - Standard：救援請求詳情頁（請求 + Helper + 地圖）
      - Complex：個人儀表板（多個獨立區塊）、戰況地圖（多個圖層）
