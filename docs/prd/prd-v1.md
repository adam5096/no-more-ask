...
4. [功能流程圖（User Flows）](#功能流程圖user-flows)
5. [功能結構圖（Information Architecture）](#功能結構圖information-architecture)
6. [Wireframe 與 UI 規格（設計規格）](#wireframe-與-ui-規格設計規格)
7. [技術規格（工程規格）](#技術規格工程規格)
8. [MVP 決策記錄](#mvp-決策記錄)
9. [驗收標準（Acceptance Criteria）](#驗收標準acceptance-criteria)
10. [修訂追蹤表（Revision History）](#修訂追蹤表revision-history)
...
flowchart TD
    ...
    Generate --> ViewScript[查看生成的腳本]
    ViewScript --> Copy[複製腳本]
    Copy --> Save{是否收藏?}
    Save -->|是| SaveScript[收藏腳本]
    Save -->|否| End([流程結束])
    SaveScript --> End
```
...
```mermaid
flowchart TD
    Start([進入系統]) --> RegisterHelper[註冊成為 Helper]
    RegisterHelper --> InputSkills[輸入技能標籤]
    InputSkills --> InputBio[輸入個人簡介]
    InputBio --> SetRate[設定時薪]
    SetRate --> Submit[提交註冊]
    Submit --> HelperRegistered[Helper 註冊成功]
    HelperRegistered --> HelperDashboard[Helper 儀表板]
    HelperDashboard --> ToggleStatus[切換為在線狀態]
    ToggleStatus --> ViewRequests[查看可接案件]
    ViewRequests --> SelectRequest[選擇救援請求]
    SelectRequest --> ViewDetails[查看請求詳情]
    ViewDetails --> Accept{是否接受?}
    Accept -->|是| AcceptRequest[接受請求]
    AcceptRequest --> InProgress[進行中]
...
**頁面路由**：
- `/helper/register` → `/helper/dashboard` → `/rescue-request/[requestId]`

---

### UrbanLoner（節慶自由人）主要流程

#### 流程 1：發起聚會並參與社群

```mermaid
flowchart TD
    Start([進入系統]) --> CreateGathering[發起聚會]
    CreateGathering --> SetLocation[設定地點]
    SetLocation --> SetTime[設定時間]
    SetTime --> SetMax[設定人數上限]
    SetMax --> Submit[提交]
    Submit --> ViewGathering[查看聚會詳情]
    ViewGathering --> Venting[同溫層牆]
    Venting --> CreatePost[發布貼文宣傳聚會]
    CreatePost --> WaitJoin[等待參與者加入]
    WaitJoin --> End([流程結束])
```
...
**頁面路由**：
- `/gathering/create` → `/gathering/[gatheringId]` → `/venting`
